...
export function SurveysPage() {
  ...
  const { surveys, loading, error, fetchSurveys, createSurvey, updateSurvey, deleteSurvey } = useSurveys()
  ...
  const handleSave = async () => {
    ...
    try {
      if (modalMode === 'create') {
        await createSurvey({ name: formName.trim(), description: formDesc.trim() })
      } else if (editing) {
        await updateSurvey(editing.id, { name: formName.trim(), description: formDesc.trim() })
      }
      closeModal()
    } catch {
    ...
    <div className="p-6 md:p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-muted mt-0.5">Create and manage all surveys.</p>
        </div>
      ...
      {!loading && error && <ErrorMessage message={error} onRetry={fetchSurveys} />}
          ...
          {surveys.map((s) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {s.description && (
                  <p className="text-sm text-muted mt-0.5 truncate">{s.description}</p>
                )}
              </div>
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                ...
                <Button variant="ghost" size="sm" onClick={() => openEdit(s)} title="Edit survey">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                  </svg>
