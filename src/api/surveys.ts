...
function parseSurveys(xml: string): Survey[] {
  const parsed = parseXml<{
    surveys: {
      survey: Array<{ "@_id": string; name: string; description: string }>;
    };
  }>(xml);
...
function parseSurvey(xml: string): Survey {
  const parsed = parseXml<{
    survey: { "@_id": string; name: string; description: string };
  }>(xml);
...
export async function getSurveys(): Promise<Survey[]> {
  const res = await client.get<string>("/api/surveys", {
    responseType: "text",
  });
...
export async function getSurvey(id: number): Promise<Survey> {
  const res = await client.get<string>(`/api/surveys/${id}`, {
    responseType: "text",
  });
...
export async function createSurvey(data: {
  ...
  const res = await client.post<string>("/api/surveys", buildSurveyXml(data), {
    responseType: "text",
  });
  return parseSurvey(res.data);
}
...
export async function updateSurvey(
  ...
  const res = await client.put<string>(
    `/api/surveys/${id}`,
    buildSurveyXml(data),
    { responseType: "text" },
  );
